...
const MoviesPage = () => {
  ...
  const { series, isPending, hasNextPage, isFetchingNextPage, fetchNextPage } = useSeries();
  ...
  if (isPending || !series) return <LoadingSkeleton />;

  return (
    <>
      <Navbar currentPath="/series" onVault={() => navigate('/vault')} />
        ...
        <div className="mx-auto max-w-[var(--layout-container-max)] px-[var(--layout-margin-desktop)] max-md:px-5">
          <div className="mb-16 space-y-4 text-center">
            <h2 className="font-display text-3xl font-bold tracking-tight text-[var(--color-on-surface)] md:text-4xl">
              Discover Series worth adding to your vault
            </h2>
          ...
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
            {series.map((card: SeriesDATA, index: number) => {
